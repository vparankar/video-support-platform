    ...
    <div className="layout-container">
      ...
      <aside className={`sidebar ${isSidebarOpen ? 'sidebar-open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          ...
          <a href="#" onClick={(e) => { e.preventDefault(); setActiveView('active'); setIsSidebarOpen(false); }} className={`nav-item ${activeView === 'active' ? 'nav-item-active' : ''}`}>
            ...
            {sortedSessions.filter(s => s.status === 'active' || s.status === 'waiting').length > 0 && (
              <span className="badge badge-amber" style={{ marginLeft: 'auto', fontSize: 10, padding: '2px 6px' }}>
                {sortedSessions.filter(s => s.status === 'active' || s.status === 'waiting').length}
              </span>
            )}
          </a>
          <a href="#" onClick={(e) => { e.preventDefault(); setActiveView('history'); setIsSidebarOpen(false); }} className={`nav-item ${activeView === 'history' ? 'nav-item-active' : ''}`}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: 6 }}>
            ...
            {sortedSessions.filter(s => s.status === 'ended').length > 0 && (
              <span className="badge badge-gray" style={{ marginLeft: 'auto', fontSize: 10, padding: '2px 6px' }}>
                {sortedSessions.filter(s => s.status === 'ended').length}
              </span>
            )}
          </a>
          <a href="#" onClick={(e) => { e.preventDefault(); setActiveView('stats'); setIsSidebarOpen(false); }} className={`nav-item ${activeView === 'stats' ? 'nav-item-active' : ''}`}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: 6 }}>
      ...
      <main className="main-content">
                    ...
                    <tbody>
                      {sortedSessions.filter(s => s.status === 'active' || s.status === 'waiting').map(sess => (
                        <tr key={sess.id}>
                          ...
                          <td>
                            <span className={`badge ${sess.status === 'active' ? 'badge-green' : 'badge-amber'}`}>
                              {sess.status}
                            </span>
                          </td>
        ...
        {activeView === 'history' && (
          <>
            <div className="page-header">
              ...
              <p className="page-subtitle">Review completed support sessions, chat logs, and recordings</p>
            </div>
            <div style={{ padding: '0 32px 32px' }}>
              <div className="card">
                <div className="card-header">
                  <span className="card-title">Completed Sessions</span>
        ...
        {activeView === 'stats' && (
          <>
            <div className="page-header">
              <h1 className="page-title">Quick Stats</h1>
              <p className="page-subtitle">Overview of your support session activity</p>
            </div>
            <div style={{ padding: '0 32px 32px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
              <div className="card" style={{ padding: '1.5rem' }}>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 8 }}>Total Sessions</div>
                <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--text)' }}>{sessions.length}</div>
              </div>
              <div className="card" style={{ padding: '1.5rem' }}>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 8 }}>Active Now</div>
                <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--green)' }}>
                  {sessions.filter(s => s.status === 'active').length}
                </div>
              </div>
              <div className="card" style={{ padding: '1.5rem' }}>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 8 }}>Waiting</div>
                <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--brand-yellow)' }}>
                  {sessions.filter(s => s.status === 'waiting').length}
                </div>
              </div>
              <div className="card" style={{ padding: '1.5rem' }}>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 8 }}>Completed</div>
            ...
            <div style={{ padding: '0 32px 32px' }}>
              <div className="card" style={{ padding: '1.5rem' }}>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 12 }}>Total Participants Served</div>
            ...
            {!inviteLink ? (
              <>
                <input
                  style={input}
                  type="text"
                  placeholder="Enter session titleΓÇª"
                  value={newTitle}
      ...
      {showChatModal && (
        <div style={overlay} onClick={closeChatModal}>
          <div style={{ ...modal, maxWidth: 560 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontSize:'1.15rem',fontWeight:600,color:'var(--text)',marginBottom:'1.25rem' }}>
              Chat History — {chatSession?.title || 'ΓÇª'}
            </h3>
            {chatLoading ? (
              <div style={{ color:'var(--text-muted)',textAlign:'center',padding:'1rem' }}>LoadingΓÇª</div>
            ) : (
